...
// state 초기값으로 원시값(숫자, 문자, boolean), 배열, 객체 등 모든 데이터 타입 사용 가능
// 주로 객체 형태로 사용함
...
const counterSlice = createSlice({
  ...
  reducers: { // reducer들을 정의
    ...
    decrement: (state) => { // 첫번재 파라미터: 현재 state를 받아옴(실제로는 state의 복사본)
      state.value -= 1; // 배열 또는 객체의 경우에도 직접 수정하는 형태로 작성 가능!!!!!!!!!!!
      // 어떻게? Immer 라이브러리 내장되어 불변성 관리를 해주기 때문
      // 실제로는 상태를 직접 변경하지 않고 내부적으로 state 복사본을 만들어서 그 복사본을 변경하고 새로운 상태를 반환함
    },
...
export const { decrement,increment,incrementByAmount } =  counterSlice.actions; // 각 리듀서에 대한 액션 생성 함수들이 객체 형태로 들어있음
...
// => dispath() 함수를 사용하여 store에 액션을 보냄
