...
function Counter(props) {
  ...
  const [colorChange, setColorChange] = useState('blue'); // 글자색 상태
  ...
  return (
    <>
      ...
      {/* <p style={{ color: colorChange}}>현재 글자색: {colorChange}</p>
      <button onClick={() => setColorChange('red')}>글자색 변경</button> */}
      <p style={{ color: colorChange}}>현재 글자색: {colorChange}</p>
      <button onClick={() => {
      ...
      <p>버튼 내용 바꾸기</p>
      <button onClick={() => setGudok(!gudok)}>{gudok ? '구독 완료' : '구독'}</button>
    </>
  );
}
